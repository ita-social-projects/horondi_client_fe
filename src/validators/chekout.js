...
export const validationSchema = (deliveryType, language) =>
  Yup.object().shape({
    ...
    city:
      ...
      Yup.string()
        .min(2, CHECKOUT_ERROR[language].city)
        .max(50, CHECKOUT_ERROR[language].city)
        .required(CHECKOUT_ERROR[language].requiredField),
    street:
      (deliveryType === deliveryTypes.NOVAPOSTCOURIER ||
        deliveryType === deliveryTypes.UKRPOSTCOURIER) &&
      Yup.string()
        .min(2, CHECKOUT_ERROR[language].street)
        .max(100, CHECKOUT_ERROR[language].street)
        .required(CHECKOUT_ERROR[language].requiredField),
    house:
      (deliveryType === deliveryTypes.NOVAPOSTCOURIER ||
        deliveryType === deliveryTypes.UKRPOSTCOURIER) &&
      Yup.string()
        .min(1, CHECKOUT_ERROR[language].house)
        .required(CHECKOUT_ERROR[language].requiredField),
    flat:
      (deliveryType === deliveryTypes.NOVAPOSTCOURIER ||
        deliveryType === deliveryTypes.UKRPOSTCOURIER) &&
      Yup.string().min(1, CHECKOUT_ERROR[language].flat)
  });
