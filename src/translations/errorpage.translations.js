export const ERROR_PAGE_MESSAGE = {
  BUSINESS_TEXT_NOT_FOUND: [
    { value: 'Текст бізнесу не знайдений' },
    { value: 'Business text not found' }
  ],
  CATEGORY_NOT_VALID: [{ value: 'Категорія не вірна' }, { value: 'Category is not valid' }],
  CATEGORY_NOT_FOUND: [{ value: 'Категорію не знайдено' }, { value: 'Category not found' }],
  NEWS_NOT_FOUND: [{ value: 'Новин не знайдено' }, { value: 'News not found' }],
  MATERIAL_NOT_FOUND: [{ value: 'Матеріал не знайдено' }, { value: 'Material not found' }],
  PRODUCT_NOT_FOUND: [{ value: 'Товар не знайдено' }, { value: 'Product not found' }],
  ORDER_NOT_FOUND: [{ value: 'Замовлення не знайдено' }, { value: 'Order not found' }],
  PATTERN_NOT_FOUND: [{ value: 'Гобелен не знайдено' }, { value: 'Pattern not found' }],
  USER_NOT_FOUND: [{ value: 'Користувача не знайдено' }, { value: 'User not found' }],
  IMAGE_NOT_FOUND: [{ value: 'Фото не знайдено' }, { value: 'Image not found' }],
  DEFAULT_ERROR: [{ value: 'Щось пішло не так...' }, { value: 'Something went wrong...' }]
};

export const LINK_TO_HOMEPAGE = [{ value: 'На головну' }, { value: 'Go to homepage' }];
