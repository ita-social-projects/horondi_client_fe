export const mockId = '6024a1aff38a1b3938db6fd9';
export const mockConstructorModel = {
  _id: '6024a1aff38a1b3938db6fd9',
  image: 'img.png',
  basePrice: [
    { value: '12345', currency: 'UAH' },
    { value: '123', currency: 'USD' }
  ]
};
export const mockError = {
  message: 'error'
};
export const methodCall = 'CALL';
export const methodPut = 'PUT';
export const mockConstructorPattern = {
  _id: '12345f3bf38a1b3938db6fd7',
  constructorImg: 'large_cc6kbawkl0a665o_гобелен-3.png'
};
export const mockConstructorModels = [
  {
    _id: '6024a1aff38a1b3938db6fd9',
    name: [{ value: 'Name_1' }, { value: 'Name_2' }]
  },
  {
    _id: '6024a1aff46a1b3938db6fd9',
    name: [{ value: 'Name_1' }, { value: 'Name_2' }]
  }
];
export const mockConstructorModelData = {
  _id: '6024a1aff38a1b3938db9fd9',
  constructorBasic: [
    {
      _id: '6024a1aff38a1b3938db6f9',
      name: [{ value: 'Name_1' }, { value: 'Name_1' }]
    },
    {
      _id: '6024s1aff38a1b3938db6f9',
      name: [{ value: 'Name_2' }, { value: 'Name_2' }]
    }
  ],
  constructorBottom: [
    {
      _id: '6024a1aff38a1b3938db6f9',
      name: [{ value: 'Name_1' }, { value: 'Name_1' }]
    },
    {
      _id: '6024s1aff38a1b3938db6f9',
      name: [{ value: 'Name_2' }, { value: 'Name_2' }]
    }
  ],
  constructorFrontPocket: [
    {
      _id: '6024a1aff38a1b3938db6f9',
      name: [{ value: 'Name_1' }, { value: 'Name_1' }]
    },
    {
      _id: '6024s1aff38a1b3938db6f9',
      name: [{ value: 'Name_2' }, { value: 'Name_2' }]
    }
  ],
  constructorFrontPattern: [
    {
      _id: '6024a1aff38a1b3938db6f9',
      name: [{ value: 'Name_1' }, { value: 'Name_1' }]
    },
    {
      _id: '6024s1aff38a1b3938db6f9',
      name: [{ value: 'Name_2' }, { value: 'Name_2' }]
    }
  ]
};
