...
const SideBarItem = ({
  ...
  const { quantityPerPage } = useSelector(
    ({ Products }) => ({
  ...
  const handleModelClick = (productModels, category) => {
    history.push('/');
    history.push(`products/?${page}=${defaultPage}&${sort}=${POPULARITY}&${countPerPage}=${quantityPerPage}&${categoryFilter}=${category}&${modelsFilter}=${productModels.name[language].value}`);
  };
  return (
    <>
